...
import {
  ...
  refreshGroups,
  reloadPropertyTree,
  initializedPropertyTree
} from '../Actions';
import { actionTypes } from '../Actions/actionTypes';
import { ConnectionStates } from '../Reducers/connection';
...
const getPropertyTree = async dispatch => {
  ...
  dispatch(initializedPropertyTree());
};
...
export const propertyTree = store => next => action => {
  ...
  if (action.type === actionTypes.setPropertyValue) {
    // Optimization: Capture the setPropertyValue action,
    // and don't let other middleware or reducers act on
    // that action, since they shouldn't act on it anyways.
    result = store;
  }
  else {
    result = next(action);
